...
namespace Movie.Api.Controllers
{
    ...
    [ApiController]
    public class Olmeden1Controller : ControllerBase
    {
        private readonly DataContext _dbContext;

        public Olmeden1Controller(DataContext dbContext)
        {
        ...
        {
            var posts = await _dbContext.Olmedens1.ToListAsync();
            return Ok(posts);
        }
        [HttpGet]
        ...
        {
            var post = await _dbContext.Olmedens1.FirstOrDefaultAsync(p => p.Id == id);
            if (post != null)
            {
                return Ok(post);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> AddPost(AddOlmedenRequest1 addOlmedenRequest)
        {
            var post = new Olmeden1()
            {
            ...
            await _dbContext.Olmedens1.AddAsync(post);
        ...
        [HttpPut]
        [Route("{id:guid}")]
        public async Task<IActionResult> UpdatePost([FromRoute] Guid id, UpdateOlmedenRequest1 updateOlmedenRequest)
        {
        ...
        {
            var existingPost = await _dbContext.Olmedens1.FindAsync(id);
            if (existingPost != null)
            {
