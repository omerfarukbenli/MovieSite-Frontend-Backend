...
namespace Movie.Api.Controllers
{
    ...
    public class SeriController : ControllerBase
    {
        ...
        public SeriController(DataContext dbContext)
        {
        ...
        {
            var posts = await _dbContext.Seris.ToListAsync();
            return Ok(posts);
        }
        [HttpGet]
        ...
        {
            var post = await _dbContext.Seris.FirstOrDefaultAsync(p => p.Id == id);
            if (post != null)
            {
                return Ok(post);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> AddPost(AddSeriRequest addSeriRequest)
        {
            var post = new Seri()
            {
            ...
            await _dbContext.Seris.AddAsync(post);
        ...
        [HttpPut]
        [Route("{id:guid}")]
        public async Task<IActionResult> UpdatePost([FromRoute] Guid id, UpdateSeriRequest updateSeriRequest)
        {

            var existingPost = await _dbContext.Seris.FindAsync(id);
