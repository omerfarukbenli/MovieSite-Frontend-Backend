...
namespace Movie.Api.Controllers
{
    ...
    [ApiController]
    public class Oscar1Controller : ControllerBase
    {
        private readonly DataContext _dbContext;

        public Oscar1Controller(DataContext dbContext)
        {
        ...
        {
            var posts = await _dbContext.Oscars1.ToListAsync();
            return Ok(posts);
        }
        [HttpGet]
        ...
        {
            var post = await _dbContext.Oscars1.FirstOrDefaultAsync(p => p.Id == id);
            if (post != null)
            {
                return Ok(post);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> AddPost(AddOscarRequest1 addOscarRequest)
        {
            var post = new Oscar1()
            {
            ...
            await _dbContext.Oscars1.AddAsync(post);
        ...
        [HttpPut]
        [Route("{id:guid}")]
        public async Task<IActionResult> UpdatePost([FromRoute] Guid id, UpdateOscarRequest1 updateOscarRequest)
        {

            var existingPost = await _dbContext.Oscars1.FindAsync(id);
