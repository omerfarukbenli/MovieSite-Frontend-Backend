...
namespace Movie.Api.Controllers
{
    ...
    [ApiController]
    public class Anasayfa1Controller : ControllerBase
    {
        private readonly DataContext _dbContext;

        public Anasayfa1Controller(DataContext dbContext)
        {
        ...
        {
            var posts = await _dbContext.Anasayfas1.ToListAsync();
            return Ok(posts);
        }
        [HttpGet]
        ...
        {
            var post = await _dbContext.Anasayfas1.FirstOrDefaultAsync(p => p.Id == id);
            if (post != null)
            {
                return Ok(post);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> AddPost(AddAnasayfaRequest1 addAnasayfaRequest)
        {
            var post = new Anasayfa1()
            {
            ...
            await _dbContext.Anasayfas1.AddAsync(post);
        ...
        [HttpPut]
        [Route("{id:guid}")]
        public async Task<IActionResult> UpdatePost([FromRoute] Guid id, UpdateAnasayfaRequest1 updateAnasayfaRequest)
        {

            var existingPost = await _dbContext.Anasayfas1.FindAsync(id);
