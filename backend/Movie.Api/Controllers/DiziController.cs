...
namespace Movie.Api.Controllers
{
    ...
    public class DiziController : ControllerBase
    {
        ...
        public DiziController(DataContext dbContext)
        {
        ...
        {
            var posts = await _dbContext.Dizis.ToListAsync();
            return Ok(posts);
        }
        [HttpGet]
        ...
        {
            var post = await _dbContext.Dizis.FirstOrDefaultAsync(p => p.Id == id);
            if (post != null)
            {
                return Ok(post);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> AddPost(AddDiziRequest addDiziRequest)
        {
            var post = new Dizi()
            {
            ...
            await _dbContext.Dizis.AddAsync(post);
        ...
        [HttpPut]
        [Route("{id:guid}")]
        public async Task<IActionResult> UpdatePost([FromRoute] Guid id, UpdateDiziRequest updateDiziRequest)
        {

            var existingPost = await _dbContext.Dizis.FindAsync(id);
