...
namespace Movie.Api.Controllers
{
    ...
    [ApiController]
    public class Hollywood1Controller : ControllerBase
    {
        private readonly DataContext _dbContext;

        public Hollywood1Controller(DataContext dbContext)
        {
        ...
        {
            var posts = await _dbContext.Hollywoods1.ToListAsync();
            return Ok(posts);
        }
        [HttpGet]
        ...
        {
            var post = await _dbContext.Hollywoods1.FirstOrDefaultAsync(p => p.Id == id);
            if (post != null)
            {
                return Ok(post);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> AddPost(AddHollywoodRequest1 addHollywoodRequest)
        {
            var post = new Hollywood1()
            {
            ...
            await _dbContext.Hollywoods1.AddAsync(post);
        ...
        [HttpPut]
        [Route("{id:guid}")]
        public async Task<IActionResult> UpdatePost([FromRoute] Guid id, UpdateHollywoodRequest1 updateHollywoodRequest)
        {

            var existingPost = await _dbContext.Hollywoods1.FindAsync(id);
